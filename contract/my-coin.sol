...
contract ERC20Basic is IERC20 {
    ...
    function increaseTotalSupply(uint256 newTotalAmount) public {
    ...
    function transfer(address _recipient, uint256 _tokensNumber)
        public
        override
        returns (bool)
    {
    ...
    function transferFrom(
        ...
        require(
            _amount <= balances[_owner],
            "El Owner no tiene suficiente dinero"
        );
        require(
            _amount <= allowed[_owner][msg.sender],
            "El Spender no tiene suficiente dinero"
        );
